    ...
    <header className="sticky top-0 z-50 backdrop-blur-sm bg-background/85 border-b border-border">
      <div className="mx-auto max-w-[1400px] px-5 md:px-10 py-4 flex items-center justify-between gap-6 flex-nowrap">
        ...
        <button
          ...
          aria-expanded={open}
          onClick={() => setOpen((v) => !v)}
          className="md:hidden inline-flex items-center justify-center w-10 h-10 -mr-2 text-foreground hover:text-bronze transition-colors"
        >
      ...
      {/* Mobile menu */}
      <div
        className={`md:hidden fixed inset-x-0 top-[57px] bottom-0 bg-background/98 backdrop-blur-md transition-opacity duration-300 ${
          open ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"
        }`}
      >
        <nav className="flex flex-col px-6 pt-10 pb-12 gap-2">
          {nav.map((item) => (
            <a
              key={item.label}
              href={item.href}
